...
#创建『线程池执行器』、使用submit 方法提交任务、使用 shutdown 方法等待任务完成。
...
#     #创建一个线程池执行器
#     executor = ThreadPoolExecutor(3)
#     #使用submit 方法提交任务(submit只负责提交任务，不会阻塞主线程)
...
#     lock = RLock()
#     #开启7个任务，并指定回调函数  map
#     #通过 map 方法批量提交任务(结果按照提交的顺序来)  map方法是不阻塞的  加了list(results)就阻塞了
#     results=executor.map(work, [1,2,3,4,5,6,7], [lock]*7)
#     print(list(results))  # 打印每个任务的结果
#     #等所有任务都完成
#     executor.shutdown(wait=True)
#     print('------------end------------')

#6.使用 with 语句创建线程池执行器
def work(n,lock):
    time.sleep(2)
    if n==1:
        time.sleep(15)
